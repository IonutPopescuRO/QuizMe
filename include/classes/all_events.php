...
class paginate
{
	...
	public function dataview($query, $search=NULL, $categories, $status, $edit)
	{
		global $site_url;
		...
		if($rowCount>0)
		{
			...
			<tr>
				...
				<th><?php print $row['name']; ?></th>
				<th>
					<?php
						if($row['expire'] != "0000-00-00 00:00:00")
						{	
							$date1 = new DateTime("now");
							$date2 = new DateTime($row['expire']);
							if($date1 < $date2)
								print $status[1];
							else print $status[2];
						} else
						{
							if($row['status']==1) print $status[1];
							else print $status[3];
						}
					?>
				</th>
